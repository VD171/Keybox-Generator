...
def main():
    ...
    vd_new_cert = subprocess.run(f"openssl x509 -req -in {vd_temp_new_csr} -CA {vd_temp_cert} -CAkey {vd_temp_key} -CAcreateserial -days {vd_args.days} -sha256", shell=True, capture_output=True, text=True).stdout.strip()
